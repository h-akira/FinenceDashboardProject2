...
金融関連のチャートを集約したダッシュボード Web アプリケーション。AWS 上にサーバーレスアーキテクチャで構築。
...
- フロントエンド: Vue 3 SPA
- バックエンド: Python, API Gateway + Lambda (SAM)
...
各ディレクトリに配置できるコンテキストファイル。ディレクトリ固有のルール・指針・影響範囲・連動ファイルなどを記載する。

- **読み方**: ファイルを編集する際、対象ファイルからプロジェクトルートまでの各ディレクトリに `.dir.md` が存在するか確認し、存在する場合は必ず読むこと。例: `aaa/bbb/ccc/hoge.py` を編集する場合、`aaa/.dir.md`、`aaa/bbb/.dir.md`、`aaa/bbb/ccc/.dir.md` の存在を確認し、存在するものはすべて読む
- **記載内容**: ディレクトリのルール・指針に加え、編集時に連動して変更が必要なファイルや影響範囲も記載する
- **更新**: 実装に伴い `.dir.md` の内容が実態と合わなくなった場合は更新すること。新しいディレクトリを作成した場合は `.dir.md` の新規作成を検討すること
- **簡潔さ**: コンテキストを圧迫しないよう最大 30 行程度に保つこと

## 開発ルール

以下の開発ルールを必ず守ること。

- **プロジェクトルートの `docs/` を唯一の正（Single Source of Truth）とする**: 各ユニットの実装は `docs/` の定義（API 契約・命名規約・通信仕様等）に厳密に従うこと
- **`.dir.md` を必ず読む**: ファイル編集時は「`.dir.md`（ディレクトリコンテキスト）」セクションに従い、対象パス上の `.dir.md` をすべて読んで従うこと
